...
from security import Messenger
...
class Scraper:
    def __init__(self):
        ...
        self.key = {}
        self.driver = None
        self.security_manager: Authorizer = None
        self.key_service:  Messenger = None
    ...
    def initChromeDriver(self, headless=False):
        ...
        if sys.platform == 'win32':
            driver = webdriver.Chrome(str(os.getcwd()) + "/.lib/chromedriver.exe", options=chrome_options, desired_capabilities=capabilities)
            # driver = webdriver.Remote("http://192.168.58.1:4444", options=chrome_options, desired_capabilities=capabilities)
